...
The existing open-source Continuum Browser Signer demonstrates secure communication between a browser and the local Continuum workspace for Nostr authentication.

Continuum Vault builds upon many of the same architectural principles while introducing a separate browser extension dedicated to legacy website authentication and autofill.

Separating these responsibilities reduces the permission scope of each extension while allowing both to evolve independently.
